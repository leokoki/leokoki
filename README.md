# 👋 Hello! I'm Leo Koki Shashiki

**💼 Data Analyst | [Databricks Certified Data Engineer Associate](https://credentials.databricks.com/9e278cb2-aa2a-4e29-88b9-4e3c8a65eef7)**

I'm a data-driven professional passionate about turning complex information into actionable insights that drive measurable business results.  
With over **5 years of experience** in analytics and data engineering, I’ve worked on projects that blend technical precision, analytical reasoning, and business impact.

At **Itaú BBA**, the largest bank in Latin America, I developed predictive models and automated analyses using **Python, SQL, and AWS**, increasing offer conversion rates by **46%** and improving churn prediction accuracy from **30% to 65%**.  
I’m recognized for combining technical depth with a strategic mindset to deliver meaningful results.

---

## 🧠 Core Skills

- **Programming & Tools:** Python, SQL, Excel, Power BI  
- **Cloud & Big Data:** AWS (Sagemaker, Athena, Glue, Quicksight), Databricks  
- **Data Engineering:** ETL, Data Modeling (Power Designer), Automation  
- **Machine Learning:** Regression, Classification, Feature Engineering  
- **Other:** Web Scraping, Data Visualization, Performance Optimization  

---

## 💼 Professional Experience

### **Data Analyst — Itaú BBA** *(Sep 2022 – Oct 2025)*  
- Built automated data models integrating internal and external sources to identify new business opportunities.  
- Led “Best Cash Conversation,” achieving **70% engagement** and **46% conversion** of offers.  
- Developed the **AplicAut alert** system in Python and AWS Glue, reducing detection time from **30 days to 2 days**.  
- Created predictive models using **AWS Sagemaker** to forecast client credit behavior.

### **Data Engineer — Itaú Unibank** *(Aug 2020 – Sep 2022)*  
- Supported Itaú’s large-scale migration to AWS, ensuring reliability and scalability.  
- Designed and optimized **logical and physical data models**, promoting data consistency.  
- Built ETL pipelines with **Python, SQL, Hive, and Alteryx**, improving performance and automation.  
- Honored in **PRAD 2022** for resolving high-impact data issues and accelerating stalled projects.

### **Data Tutor — DNC School** *(Sep 2022 – Oct 2022)*  
- Mentored students in **Python, Power BI, and Excel**, helping them develop real-world analytical skills.  
- Recognized for clear explanations and engaging teaching methods.

---

## 🎓 Education & Certifications

🎓 **MBA in Data Science** – Sirius Education *(2023–2024)*  
🎓 **B.Sc. Automation and Control Engineering** – FEI University *(2015–2020)*  
📜 **[Databricks Certified Data Engineer Associate](https://credentials.databricks.com/9e278cb2-aa2a-4e29-88b9-4e3c8a65eef7)** – Databricks Academy *(2026)*  

---

## 🌍 Languages

- 🇧🇷 **Portuguese:** Native  
- 🇬🇧 **English:** Advanced (C1)  
- 🇪🇸 **Spanish:** Basic (A1)

---

## 🏆 Achievements

- 🥇 **PRAD 2022 – Itaú Unibank:** Recognized for outstanding performance in data modeling and process optimization.  
- 📘 **EUSFLAT 2019 Publication:** Research on financial market pattern classification using the Modified Gustafson-Kessel algorithm.

---

## 🔗 Connect with Me

💼 **LinkedIn:** [linkedin.com/in/leo-koki-shashiki](https://www.linkedin.com/in/leo-koki-shashiki/)  
🧠 **Portfolio:** [Portfolio Leo Koki Shashiki](https://sites.google.com/view/portifolio-leo-koki-shashiki/home)  
📧 **Email:** [leo.koki.shashiki@gmail.com](mailto:leo.koki.shashiki@gmail.com)

---

⭐ *“Data is powerful only when transformed into insight.”*  
